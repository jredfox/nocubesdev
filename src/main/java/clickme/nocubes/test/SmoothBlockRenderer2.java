...
public class SmoothBlockRenderer2 implements ISimpleBlockRenderingHandler
{
    ...
    public static float getSmoothBlockHeightForCollision(final World access, final Block block, final int x, final int y, final int z)
    {
        boolean flag = false;
        for(int j = 0; j < 4; ++j)
        {
            final int k = x - (j & 0x1);
            final int l = z - (j >> 1 & 0x1);
            IBlockState state = access.getBlockState(new BlockPos(k, y + 1, l));
            ...
            final IBlockState state2 = access.getBlockState(new BlockPos(k, y, l));
            final Block block2 = state2.getBlock();
            final Material material2 = block2.getMaterial(state2);
